...
export class Simulation{
    ...
    apply_normal_penalty(particle, normal, depth){
        normal = normal.normalized()
        let normal_velocity_component = particle.velocity.dot(normal)
        let normal_position_component = depth
        const damping = (this.ground_kd * normal_velocity_component) ** 2 * Math.sign(this.ground_kd * normal_velocity_component);
        let force = normal_position_component * this.ground_ks - this.ground_kd * normal_velocity_component;

        particle.ext_force = particle.ext_force.plus(normal.times(force))
    }
    ...
    update(){
        ...
        for(let i = 0; i < plen; i++) {
            ...
            Rigidbody.searchableObjects.forEach((obj) => {
                obj.colliders.forEach((col) => {
                    let result = col.checkCollision(particle.position)
                    if(result.detected === true) {
                        this.apply_normal_penalty(particle, result.normal, result.pendepth);
                        particle.velocity = particle.velocity.plus(result.vel.times(0.01))
                    }
                })
            })
    ...
    draw(webgl_manager, uniforms, material, debug_shape=null) {
        ...
        //     debug_shape.draw(webgl_manager, uniforms, Mat4.translation(...p.position).times(Mat4.scale(0.2, 0.2, 0.2)), material)
        // })
...
export class Particle{
    constructor() {
        this.mass = 1;
        this.position = vec3(0, 5, 0).randomized(1);
