...
class Voxels{
    ...
    cube_meshes(encoded_mp){
        ...
        for(let i = 0; i < 8; i++) {
            if (this.data.has(corners[i])) {
                ...
                if(this.data.get(corners[i]) >= 1) pIndex += 1 << i
            }else{
    ...
    interp_edge(corners, edge){
        ...
        let interp = (1 - p1val) / (p2val - p1val)
...
class MarchingSquares extends Shape {
    constructor() {
        ...
        this.arrays.position = new Array(100000).fill(vec3(0,0,0))
        this.arrays.normal = new Array(100000).fill(vec3(0,0,0))
        this.indices = Array.from(Array(100000).keys())
    }
